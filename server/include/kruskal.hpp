...
private:
    ...
    // 디버깅을 위한 연결된 컴포넌트 출력
    void debugComponentData(const unordered_map<int, pair<int, int>>& componentData, const vector<tuple<int, int, int>>& mst) {
        cout << "디버깅: 연결된 컴포넌트 정보\n";
        for (const auto& [root, data] : componentData) {
            ...
            cout << "연결된 간선 정보:\n";
            for (const auto& [from, to, cost] : mst) {
                if (Find(from) == root || Find(to) == root) {
                    cout << "  간선: " << from << " - " << to << " (비용: " << cost << ")\n";
                }
            }
            cout << "--------------------------------------\n";
        }
    ...
    int calculateMSTAverageCost() {
        ...
        for (const auto& [u, v, weight] : edges) {
            if (Find(u) != Find(v)) { // 같은 집합이 아니면 간선 추가
                ...
                componentData[rootU].second++;       // 간선 수 증가
            }
        }

        // 디버깅: 연결된 컴포넌트 정보 출력 (간선 포함)
        debugComponentData(componentData, mst);
